...
export default function AdminUsers() {
  ...
  const subCounts = {
    active: users.filter(u => u.plan === 'premium' || u.plan === 'elite').length,
    inactive: users.filter(u => u.plan === 'free').length,
    pending: users.filter(u => u.plan !== 'free' && u.plan !== 'premium' && u.plan !== 'elite').length,
  }
  ...
  const filtered = users.filter(u =>
    ...
    (subFilter === 'all' ||
      (subFilter === 'active' && (u.plan === 'premium' || u.plan === 'elite')) ||
      (subFilter === 'inactive' && u.plan === 'free') ||
      (subFilter === 'pending' && u.plan !== 'free' && u.plan !== 'premium' && u.plan !== 'elite')) &&
  ...
  const getSubStatus = (plan) => {
    if (plan === 'premium' || plan === 'elite') return { label: t('adminUsers', 'subActive'), color: 'bg-emerald-100 text-emerald-700' }
    if (plan === 'free') return { label: t('adminUsers', 'subInactive'), color: 'bg-gray-100 text-gray-500' }
