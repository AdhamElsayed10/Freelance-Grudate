...
export default function CompanyProfile() {
  ...
  const [form, setForm] = useState({
    name: company?.name || '',
    email: company?.email || '',
    category: company?.category || 'food',
    city: company?.city || '',
    emoji: company?.emoji || '🏢',
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select name="category" value={form.category} onChange={handleChange} className="w-full bg-cream border border-gold/20 rounded-xl px-12 py-3.5 text-dark outline-none focus:border-gold/60 transition-all appearance-none cursor-pointer">
                        <option value="medical">{t('companyProfile', 'medical')}</option>
                        <option value="gym">{t('companyProfile', 'sports')}</option>
                        <option value="food">{t('companyProfile', 'restaurants')}</option>
                        <option value="fun">{t('companyProfile', 'entertainment')}</option>
                      </select>
                    </div>
                  </div>
                  <div>
